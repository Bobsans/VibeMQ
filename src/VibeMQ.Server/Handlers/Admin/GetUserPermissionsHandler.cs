...
public sealed partial class GetUserPermissionsHandler(IAuthRepository repository, ILogger<GetUserPermissionsHandler> logger) : ICommandHandler {
    ...
    public async Task HandleAsync(ClientConnection connection, ProtocolMessage message, CancellationToken cancellationToken = default) {
        ...
        // Superusers can query any user; regular users can only query their own permissions
        if (!connection.IsSuperuser &&
            ...
            await connection.SendErrorAsync(message.Id, "NOT_AUTHORIZED", "You may only view your own permissions.", cancellationToken).ConfigureAwait(false);
            return;
        }

        var permissions = await repository.GetPermissionsAsync(username, cancellationToken).ConfigureAwait(false);

        LogGetPermissions(connection.Username ?? "<superuser>", username);
